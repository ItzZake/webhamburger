async function loadSidebar() {
	try {
		...
		// Reading offsetWidth is a common way to do this
		...
		// Setup theme toggle after sidebar is loaded - use a small delay to ensure DOM is ready
		setTimeout(() => {
			if (typeof setupThemeToggle === 'function') {
				setupThemeToggle();
			} else {
				// If function not available yet, try again
				setTimeout(() => {
					if (typeof setupThemeToggle === 'function') {
						setupThemeToggle();
					}
